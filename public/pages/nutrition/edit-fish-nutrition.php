...
<div class="admin-feature-container w-full h-screen overflow-auto px-3">
    <?php include PAGES_PATH . '/partials/body-header.php' ?>
        <div class="contents-container w-[70%] mt-[-60px] bg-slate-100 mx-auto rounded-lg"> 
            ...
            <form action="/nutritions/update/<?= $fishID ?>/<?= $editNutrition->getId() ?>" method="post">
                <div class="add-family-name bg-gradient-to-bl from-green-400 via-green-500 to-green-600 b p-2 flex justify-center gap-2 items-center text-center mt-5">
                    <label for="nutrition_name" class="text-left text-sm font-bold text-slate-100">Edit Fish Nutrition:</label>
                    <input type="text" name="nutrition_name" placeholder="Enter Nutrition" value="<?= $editNutrition->getNutrition_name() ?>" id="nutrition_name" class="bg-slate-100 border border-gray-300 text-gray-900 text-xs rounded-lg focus:ring-blue-500 focus:border-blue-500 p-1 px-2 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500 ml-3 w-[40%]" required>
                    <input type="submit" value="Update" class="transition-all font-semibold text-gray-700 py-1 px-4 text-sm rounded-md cursor-pointer bg-white hover:bg-gradient-to-tr from-teal-400 via-teal-500 to-teal-600 hover:text-white">
                </div>
            </form>
