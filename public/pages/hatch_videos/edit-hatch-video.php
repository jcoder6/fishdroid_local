        ...
        <div class="contents-container w-full mt-[-58px] bg-slate-100 mx-auto px-10 py-5 min-h-screen"> 
            ...
            <div class="container flex gap-5 mt-5">
                ...
                <div class="w-[65%] border border-gray-300 rounded-md p-5">
                    ...
                    <form action="/hatchVideos/update/<?= $hatchFish->getId() ?>/<?= $editVideo->getId() ?>" method="post" class="w-full flex gap-1 item border border-gray-300 p-2 rounded-md">
                        <label for="phase_num" class="text-left mb-2 text-sm font-medium text-gray-900 mt-1 mr-1">Phase number:</label>
                        <input type="number" name="phase_num" value="<?= $editVideo->getPhase_num() ?>" class="bg-slate-100 border border-gray-300 text-gray-900 text-xs rounded-md focus:ring-blue-500 focus:border-blue-500 p-1 px-2 w-[7%]" required>
                        <input type="text" name="video_link" value="<?= $editVideo->getVideo_link() ?>" class="bg-slate-100 border border-gray-300 text-gray-900 text-xs rounded-md focus:ring-blue-500 focus:border-blue-500 p-1 px-2 w-[62%]" required>
                        <input type="submit" value="Update Phase" class="transition-all font-semibold text-gray-700 py-1 px-4 text-xs rounded-md cursor-pointer bg-white hover:bg-gradient-to-tr from-teal-400 via-teal-500 to-teal-600 hover:text-white w-[15%]">
                    </form>
                </div>
            </div>
